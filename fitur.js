...
_*SILAHKAN PILIH DIBAWAH YA KAK *_
${fakemenu}`
but = [{ buttonId: `${prefix}allmenu`, buttonText: { displayText: 'ALL MENU' }, type: 1 },{ buttonId: `${prefix}c`, buttonText: { displayText: 'SELECT MENU' }, type: 1 },{ buttonId: `${prefix}`, buttonText: { displayText: 'STORE MENU' }, type: 1 }]
...
but = [{ buttonId: `${prefix}sewabot`, buttonText: { displayText: 'sewa bot' }, type: 1 }]
...
const vcard = 'BEGIN:VCARD\n'  
  ...
  titid = 'Hallo kak, itu owner ku, jangan di ganggu ya\nbtw mau tau soal apa tentang owner ku?'
          ...
          {buttonId: `${prefix}ig`, buttonText: { displayText: `INSTAGRAM`, }, type: 1, },
            ...
            if (!isRegist) return freply(mess.daftar)
            if (isBanned) return freply(mess.banned)
               sendKontak(from, `6285701662971`, `Azura Store`, 'Sibukk!!')
               await sleep(1000)
...
break
        ...
        case 'bc2':
             ...
             for (let _ of anu) {
             	tes = `${body.slice(4)}`
             	Ramdani.sendMessage(_.jid, bc, { contentText: `${tes}`, footerText: `_*${namabot} Broadcast*_`, buttons: [{buttonId: `${prefix}bot`,buttonText:{displayText: '𝐌𝐞𝐧𝐮'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff10, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             freply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             	textt = `${body.slice(4)}`
             Ramdani.sendMessage(_.jid, { contentText: `${textt}`, footerText: `_*${namabot} Broadcast*_`, buttons: [{buttonId: `${prefix}bot`,buttonText:{displayText: '𝐌𝐞𝐧𝐮'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff10, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             freply('Suksess broadcast')}
             break
//SERTI MENU
case 'serti1':
